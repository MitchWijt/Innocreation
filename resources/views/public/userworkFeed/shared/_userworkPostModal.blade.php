<div class="modal userWorkPostModal o-scroll" id="userWorkPostModal" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
    ...
    <div class="modal-dialog modal-lg fixed-modal modal-dialog-centered custom-dialog-margin" role="document">
        <div class="modal-content modal-content-border">
            ...
            <? if($user->id != $userWorkPost->user_id) { ?>
                <div class="row switchDiv">
                    <div class="col-6">
                        <label class="switch switch_type2 m-t-10" role="switch">
                            <input data-toggle="popover" <? if ($user->hasSwitched()) echo "checked disabled"; ?> data-content='<?= \App\Services\UserConnections\ConnectionService::getPopoverSwitchView($user)?>' type="checkbox" class="switch__toggle popoverSwitch">
...
<script>
    $('.popoverSwitch').popover({ trigger: "manual" , html: true, animation:false, placement: 'auto'});
